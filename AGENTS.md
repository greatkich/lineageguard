...
The objective is not to maximize feature count. The objective is to deliver the most credible, polished, reproducible three-minute hackathon demo while keeping the codebase architecturally clean enough to continue after the event.
...
2. `docs/WINNING_STRATEGY.md`
3. `docs/ARCHITECTURE.md`
4. `docs/DEMO_STORYBOARD.md`
...
- Application/demo database: PostgreSQL.
- DataHub's own MySQL/Kafka/Elasticsearch services remain separate from the demo PostgreSQL instance.
- Formatting and linting: Biome unless an approved ADR changes the choice.
- Unit and integration tests: Vitest.
- Browser tests: Playwright.
- Do not add Redis, Temporal, Kubernetes, LangGraph, or a second backend framework without an ADR proving that the primary demo cannot be delivered without it.
...
10. The final demo path must be deterministic and replayable.
...
- `tools/datahub` seeds and ingests demo metadata but is not a second application backend.
...
- Take Playwright screenshots of the canonical demo states.
- A final demo may not depend on unit-test mocks for DataHub. Mocks are allowed only below integration level.
...
- Every visible number must come from run state or a checked demo fixture.
- Support the target recording viewport first: 1440 × 900.
...
- `docs/DEMO_STORYBOARD.md` when timing or visuals change;
