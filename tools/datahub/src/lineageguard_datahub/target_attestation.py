...
TARGET_MARKER_URN = make_dataplatform_instance_urn(
    "datahub", "lineageguard-canonical-target-attestation-v1"
)
TARGET_MARKER_NAMESPACE = "io.lineageguard.target-attestation/v1"
TARGET_MARKER_NAME = "LineageGuard canonical target attestation"
TARGET_MARKER_DESCRIPTION = (
    "Immutable bootstrap marker for the canonical LineageGuard DataHub target."
)
...
class TargetInstanceBinding:
    ...
    def target_fingerprint(self) -> str:
        payload = f"{self.canonical_url}|{self.live_instance_id}"
...
class TargetInstanceStateStore:
    ...
    def create(self) -> TargetInstanceBinding:
        self._prepare_parent()
        binding = TargetInstanceBinding(
            schema_version=TARGET_STATE_SCHEMA_VERSION,
...
def prepare_target_bootstrap(
    ...
    *,
    ...
    if canonical_url != CANONICAL_GMS_URL:
        raise ValueError("CANONICAL_DATAHUB_GMS_URL_REQUIRED")
    occupied = tuple(urn for urn in graph.allowed_mutation_urns if reader.exists(urn))
    if occupied:
        raise ValueError(f"TARGET_BOOTSTRAP_CANONICAL_URN_EXISTS:{occupied[0]}")
    marker = _read_marker(reader)
    ...
    return TargetBootstrapPlan(
        binding=binding or state_store.create(),
