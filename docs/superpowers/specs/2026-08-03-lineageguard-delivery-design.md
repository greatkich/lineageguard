...
- The repository-local DataHub skill install is incomplete: upstream requires `skills/shared-references`, and that directory is missing even though the installed skills reference it. Reinstall from the official repository and commit the complete, attributable skill set before relying on it.
...
| DataHub agent skills | Official Codex install is `npx skills add datahub-project/datahub-skills -a codex`; manual installs must include `shared-references`. | Reinstall from a pinned upstream commit and record it in `skills-lock.json`. |
...
|---|---|---|---|
| C1 | `.codex/config.toml.example`, `scripts/bootstrap-agent-tooling.sh`, and an older DataHub repository README use `npx @acryldata/mcp-server-datahub`; npm returns 404 and current DataHub docs use `uvx mcp-server-datahub`. | Current versioned DataHub documentation wins. | F0 updates examples/bootstrap to `uvx --from mcp-server-datahub==0.6.0 mcp-server-datahub`; ADR-003 records the external Python MCP process. |
...
| DataHub skills remain incomplete | High / Medium | F0 skill validation | Reinstall pinned upstream including shared references | Do not depend on skills at runtime; retain MCP path |
