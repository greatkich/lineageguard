...
- Python is a narrow tooling layer for DataHub ingestion and demo metadata seeding.
- DataHub is the context plane.
- PostgreSQL is the demo data store and LineageGuard control store.
...
The hackathon deployment fits on one sufficiently sized VPS.
...
> The application and demo can use one PostgreSQL instance with multiple schemas, but DataHub's internal storage and messaging services remain the official DataHub stack. “One database” does not mean forcing DataHub into the demo PostgreSQL database.
...
The final video should use live GitHub where stable and retain replay mode as a contingency.
...
- exists only as a recording/judging contingency.
...
- health checks;
- scheduled backup/snapshot before recording;
- hosted demo kept running through judging.
...
AWS documentation or infrastructure tools may help deploy, but AWS is not part of the judging story and must not consume core build time.
