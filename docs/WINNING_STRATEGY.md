...
The submission wording should be explicit:
...
Judges may spend most of their time on the video, README, screenshots, and examples. Therefore:
...
- the hosted demo remains available through judging.
...
Open an upstream pull request only after the project workflow is stable. A low-quality contribution can distract from the core submission.
...
| The demo is reproducible | replay command and clean-start instructions |

## Scope priority

### P0 — must exist

- controlled demo graph;
...
`pnpm demo:verify` must run the canonical scenario without manual repairs and produce the expected evidence bundle.

No stable replay, no recording.
...
| Demo is too technical | Use one human-readable rename and a clear three-stage UI |
...
LineageGuard wins when a judge can say:
