...
- protect demo narrative and judging alignment.
...
- deployment health;
- examples and README;
- video timing and captions;
- Devpost field completeness;
- public URL availability through judging.
...
Long DataHub integration tests may run in a dedicated workflow or nightly/manual job until stable, but the canonical demo smoke must run before feature freeze.
...
feat(demo): seed canonical warehouse and dbt graph
...
chore(demo): add deterministic replay and submission artifacts
```
...
- mocking the winning path in the final demo;
