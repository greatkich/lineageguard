...
- the demo can fail unpredictably;
...
- multi-agent delegation is not itself a judging criterion;
...
The hackathon rewards technical execution and real-world usefulness. A controlled agent that produces a verified PR is more credible than an impressive but unpredictable swarm. The agentic value remains clear: DataHub context changes the plan and the model produces artifacts that deterministic systems then verify.
